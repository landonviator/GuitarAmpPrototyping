# GuitarAmpPrototyping

![alt text](https://github.com/landonviator/GuitarAmpPrototyping/blob/main/amp.png "Diode Amplifier")
 
 
I'm pretty proud of this one! This is my first amp sim. It's geared towards metal guitar tones. In the future, I'd like to make a version that has different tone stacks, not just for metal. This plugin utilizes a simple signal flow: Filtering for tone shaping -> Clipping stage -> amp tone filters -> convolution for speaker cab. The clipping stage uses a custom algorithm that combines a simple diode clipper with a soft clipper, which worked out pretty well. I still need to do a lot of experimenting to see what combinations of clipping algorithms makes the guitar distortion sound natural. There's also only one IR built into the plugin. I'd obviously like to make a file loading system so the user can load their own IR's.
